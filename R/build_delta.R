...
build_delta <- function(thresh = c(50, 95),
  ...
  hist <- terra::rast(system.file("extdata",
                                  paste0(model2, "_persistence_", thresh, "_",
                                         min(histrange), "-", max(histrange), ".tif"),
                                  package = "abalone"))
  proj <- terra::rast(system.file("extdata",
                                  paste0(model2, "_persistence_", thresh, "_",
                                         min(projrange), "-", max(projrange), ".tif"),
                                  package= "abalone"))
  ...
  if (!is.null(save_path)) {
    ...
    save_pathh <- paste0(save_path, "/", model2, "_delta_", thresh,
    ...
    dev_path <- file.path(usethis::proj_path(),
                          "inst", "extdata",
                          paste0(save_path, "/", model2, "_delta_", thresh,
                                 "_", min(projrange), "-", max(projrange), "minus",
                                 min(histrange), "-", max(histrange), ".tif"))
...
#   deltarast(thresh = 50, model = i)
#   deltarast(thresh = 95, model = i)
# }
